...
private:
	...
	//目標数と現在のカウント数
	std::array<std::array<std::unique_ptr<Sprite>, 2>, GameColor::kMaxColor - 1> countNumSprites_;
	std::array<std::array<std::unique_ptr<Sprite>, 2>, GameColor::kMaxColor - 1> goalNumSprites_;
	//ブロック画像
	...
	//ゴールカウント
	std::array<int32_t, 3> goalCount_;

	//現在のカウント
	std::array<int32_t, 3> currentCount_;
